...
const manifest=JSON.parse(fs.readFileSync("/work/profile/manifest.json","utf8"));
const expected=manifest.skills.length;
let pass=0,fail=0; const chk=(n,c)=>{console.log((c?"PASS":"FAIL")+" "+n); c?pass++:fail++;};
chk("all "+expected+" manifest skills installed ("+skills.length+")", skills.length===expected);
