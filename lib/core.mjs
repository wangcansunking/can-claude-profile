...
// Pure state reducer for a checkbox picker — no I/O, fully unit-testable.
// state: { cursor: int, checked: bool[] }. Returns a NEW state.
// Recognized keys: "up"/"down" move, "space" toggles current, "a" toggles all,
// "none" clears all, "all" checks all. Unknown keys are no-ops.
export function multiSelectReducer(state, key, n) {
  const s = { cursor: state.cursor, checked: state.checked.slice() };
  switch (key) {
    case "up": s.cursor = (s.cursor - 1 + n) % n; break;
    case "down": s.cursor = (s.cursor + 1) % n; break;
    case "space": s.checked[s.cursor] = !s.checked[s.cursor]; break;
    case "a": { const allOn = s.checked.every(Boolean); s.checked = s.checked.map(() => !allOn); break; }
    case "all": s.checked = s.checked.map(() => true); break;
    case "none": s.checked = s.checked.map(() => false); break;
    default: break;
  }
  return s;
}

// Translate a raw stdin byte buffer into a logical key for multiSelectReducer,
// or "enter"/"cancel"/null. Handles arrow escape sequences, j/k, space, a, q.
export function decodeKey(buf) {
  const s = buf.toString("utf8");
  if (s === "\r" || s === "\n") return "enter";
  if (s === "\x03" || s === "\x1b") return "cancel";      // Ctrl-C / lone ESC
  if (s === " ") return "space";
...
// Render the checkbox list to a string (exported for tests / non-TTY preview).
export function renderMultiSelect(items, state, opts = {}) {
  const { cursor, checked } = state;
  const lines = items.map((label, i) => {
    const box = checked[i] ? C.green("[x]") : "[ ]";
    const pointer = i === cursor ? C.cyan("❯") : " ";
    const text = i === cursor ? C.bold(label) : label;
    return ` ${pointer} ${box} ${text}`;
  });
  return lines.join("\n");
}

// Interactive checkbox multi-select over a raw TTY. All items start checked.
// Returns the array of selected indices, or null if the user cancels.
// Falls back to "all selected" when stdin is not an interactive TTY (so
// piped/non-TTY runs keep the safe default).
export function multiSelect(items, { title, hint } = {}) {
  const n = items.length;
  if (!n) return [];
  const allIdx = items.map((_, i) => i);
  const stdin = process.stdin;
  if (!stdin.isTTY || typeof stdin.setRawMode !== "function") {
    // Non-interactive: keep everything (matches prior safe default).
    return allIdx;
  }

  let state = { cursor: 0, checked: items.map(() => true) };
  const header = (title || "Select items") +
    (hint ? "  " + C.dim(hint) : C.dim("  (↑/↓ move · space toggle · a all/none · enter confirm)"));

  const ESC = "\x1b";
  const draw = (first) => {
    if (!first) process.stdout.write(`${ESC}[${n + 1}A`); // move up to redraw
    process.stdout.write(`${ESC}[0J`);                    // clear below
    process.stdout.write(C.bold(header) + "\n" + renderMultiSelect(items, state) + "\n");
  };
  ...
  try {
    while (true) {
      let nread;
      try { nread = fs.readSync(stdin.fd, buf, 0, 8, null); }
      catch (e) { if (e.code === "EAGAIN") continue; throw e; }
      if (!nread) break;
      const key = decodeKey(buf.slice(0, nread));
      if (key === "enter") { return state.checked.map((c, i) => (c ? i : -1)).filter((i) => i >= 0); }
      if (key === "cancel") return null;
      if (key) { state = multiSelectReducer(state, key, n); draw(false); }
    }
  } finally {
    stdin.setRawMode(false);
    stdin.pause();
  }
  return state.checked.map((c, i) => (c ? i : -1)).filter((i) => i >= 0);
}
