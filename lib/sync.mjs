...
import {
  ...
  dirsEqual, diffMaps, selectItems, argListValue, argIdSet, readLineSync, multiSelect,
} from "./core.mjs";
...
// Interactive item picker: a checkbox multi-select over the actionable diff ids,
// all checked by default. ↑/↓ move, space toggles, a = all/none, Enter applies.
// Returns the Set of ids to keep. Cancel (Ctrl-C/ESC/q) keeps nothing → the
// caller treats an empty set as "write nothing". Non-TTY stdin keeps all.
function pickItemsInteractively(actionableIds) {
  const ids = [...actionableIds];
  if (!ids.length) return new Set(ids);
  const chosen = multiSelect(ids, {
    title: "Select changes to sync",
    hint: "(↑/↓ move · space toggle · a all/none · enter confirm)",
  });
  if (chosen === null) return new Set(); // cancelled
  return new Set(chosen.map((i) => ids[i]));
}
...
export function runSync() {
  ...
  let selected = selectItems(allActionable, pick, drop);
  ...
  // Interactive item picker (checkbox multi-select): when running interactively
  // with no explicit --pick/--drop, let the user toggle individual changed items.
  // Its Enter doubles as the write confirmation, so we skip the y/n below.
  const noExplicitSelection = !pick && !drop;
  const interactive = !process.argv.includes("--yes") && !process.argv.includes("-y") && !process.argv.includes("--dry-run");
  let pickerRan = false;
  if (noExplicitSelection && interactive && allActionable.length) {
    selected = pickItemsInteractively(allActionable);
    pickerRan = true;
  }
  ...
  // The checkbox picker's Enter already served as confirmation; only ask a
  // separate y/n when we didn't run it (flag-driven or settings/claudemd-only).
  if (!pickerRan && !confirm("Write the selected changes into the repo?", true)) {
