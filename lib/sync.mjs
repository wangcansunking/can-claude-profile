...
// Interactive item picker: given the actionable diff ids (all selected by
// default), show a numbered list and let the user EXCLUDE items by number
// (Enter = keep all), mirroring install's component menu but at item level.
// Returns the Set of ids to keep. Only called when interactive with no explicit
// --pick/--drop and a readable stdin.
function pickItemsInteractively(actionableIds) {
  const ids = [...actionableIds];
  if (!ids.length) return new Set(ids);
  console.log(C.bold("\nSelect changes to sync") + C.dim("  (Enter = all; or type numbers to EXCLUDE, e.g. 2 4)"));
  ids.forEach((id, i) => console.log(`  ${C.cyan(String(i + 1))}. ${id}`));
  process.stdout.write(C.yellow("? ") + "Exclude which? ");
  const line = readLineSync();
  if (line === null || !line.trim()) return new Set(ids);
  const drop = new Set(
    line.split(/[,\s]+/).map((s) => parseInt(s, 10)).filter((n) => n >= 1 && n <= ids.length));
  return new Set(ids.filter((_, i) => !drop.has(i + 1)));
}
...
export function runSync() {
  ...
  // Interactive item picker (parity with install's menu): when running
  // interactively with no explicit --pick/--drop, let the user exclude
  // individual changed items by number before writing.
